...
# Map version codes to KDF method names and vice versa
KDF_VERSION_TO_METHOD = {
    "A1dFg9X": "argon2id",
    "B8kLp7Q": "pbkdf2",
    "C3xRt1Z": "scrypt",
    "D6qNv4M": "bcrypt"
}
KDF_METHOD_TO_VERSION = {v: k for k, v in KDF_VERSION_TO_METHOD.items()}
            ...
            if parts[0] == "NOPASS":
               ...
               # OLD FORMAT — no HMAC, just decrypt as usual
                ...
                if not hmac.compare_digest(computed_hmac, hmac_sig):
                    raise ValueError("HMAC verification failed. Password Could be wrong")
